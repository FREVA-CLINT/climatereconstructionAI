...
class NetCDFLoader_lazy(Dataset):
    def __init__(self, files_source, 
        ...
        self.files_target_past = files_source_past
        ...
        self.files_source_past = files_source_past
    ...
    def get_files(self, file_path_source, file_path_target=None, file_path_source_past=None):
      
        include_past = True if file_path_source_past is not None else False
        ...
        if include_past:
            if self.lazy_load:
                ds_source_past = xr.open_dataset(file_path_source_past)
            else:
                ds_source_past = xr.load_dataset(file_path_source_past)
        ...
        while not_condition:
            ...
            if include_past:
                ds_source_past_sampled = self.apply_spatial_dim_indices(ds_source_past, self.dims_variables_source, spatial_dim_indices_source, rel_coords_dict=rel_coords_dict_source)
            else:
                ds_source_past_sampled=None
        ...
        return ds_source_sampled, ds_target_sampled, patch_id, spatial_dim_indices_source, spatial_dim_indices_target, rot_angle, ds_source_past_sampled
    ...
    def __getitem__(self, index):
        ...
        source_file_past = self.files_source_past[source_index] if self.files_source_past is not None else None

        ds_source, ds_target, patch_id, spatial_dim_indices_source, spatial_dim_indices_target, rot_angle, ds_source_past = self.get_files(source_file, file_path_target=target_file, file_path_source_past=source_file_past)
        ...
        if ds_source_past is not None:
            data_past, _ = self.get_data(ds_source_past, index, self.dims_variables_source, depth=depth_idx)
        ...
        if self.normalization is not None:
            ...
            if ds_source_past is not None:
                ds_source_past = self.normalizer(data_past)
...
class SampleLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data = [source, target, torch.zeros((10,)), coords_target, target_indices, depth]
