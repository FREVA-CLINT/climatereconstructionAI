...
def create_evaluation_maps(gt, outputs):
    ...
    for i in range(len(map_lists)):
        ...
        for j in range(len(map_lists[i])):
            ...
            plt.ylabel("km")
            ax = plt.gca()
            ax.set_yticks([i for i in range(cfg.image_sizes[0]) if i % 50 == 0])
            ax.set_yticklabels([i for i in range(cfg.image_sizes[0]) if i % 50 == 0])
            ax.set_xticks([i for i in range(cfg.image_sizes[0]) if i % 50 == 0])
            ax.set_xticklabels([i for i in range(cfg.image_sizes[0]) if i % 50 == 0])
