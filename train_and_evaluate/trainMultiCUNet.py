...
from utils.netcdfloader import PrevNextNetCDFDataLoader as NetCDFDataLoader, InfiniteSampler
...
dataset_train = NetCDFDataLoader(cfg.data_root_dir, cfg.mask_dir, 'train', cfg.data_type, cfg.prev_next)
dataset_val = NetCDFDataLoader(cfg.data_root_dir, cfg.mask_dir, 'val', cfg.data_type, cfg.prev_next)
iterator_train = iter(data.DataLoader(dataset_train, batch_size=cfg.batch_size,
                                      sampler=InfiniteSampler(len(dataset_train)),
                                      num_workers=cfg.n_threads))

# define network model
model = MultiChannelPConvUNet(image_size=cfg.image_size,
                              num_enc_dec_layers=cfg.encoding_layers,
                              num_pool_layers=cfg.pooling_layers,
                              num_in_channels=2*cfg.prev_next + 1).to(cfg.device)
