...
def create_snapshot_image(model, dataset, filename):
    ...
    with torch.no_grad():
        data_dict["output"], _ = model(data_dict["image"], data_dict["in_mask"])
...
def infill(model, dataset, eval_path, output_names, steady_mask, data_stats, xr_dss, i_model):
    ...
    for split in tqdm(range(dataset.__len__())):
        ...
        with torch.no_grad():
            data_dict["output"], _ = model(data_dict["image"].to(cfg.device), data_dict["mask"].to(cfg.device))
...
def create_outputs(data_dict, eval_path, output_names, data_stats, xr_dss, i_model, split, index, ds_index=0):
    ...
    for j in range(len(eval_path)):
        ...
        for cname in cnames:
            ...
            ds = xr_dss[i_data][ds_index]["ds1"].copy()
            dims = xr_dss[i_data][ds_index]["dims"].copy()
            coords = xr_dss[i_data][ds_index]["coords"].copy()
            coords["time"] = xr_dss[i_data][ds_index]["ds"]["time"].values[index]
            ...
            ds = reformat_dataset(xr_dss[i_data][ds_index]["ds"], ds, data_type)

            for var in xr_dss[i_data][ds_index]["ds"].keys():
                if "time" in xr_dss[i_data][ds_index]["ds"][var].dims:
                    ds[var] = xr_dss[i_data][ds_index]["ds"][var].isel(time=index)
                else:
                    ds[var] = xr_dss[i_data][ds_index]["ds"][var]
